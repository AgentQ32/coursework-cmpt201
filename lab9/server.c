...
void *handle_client(void *arg) {
  ...
  printf("New client created! ID %d on socked FD %d\n", client->client_id, client->cfd);

  // print message received from client
  while ((read_data = read(client->cfd, buf, BUF_SIZE - 1)) > 0) {

    if (write(STDOUT_FILENO, buf, read_data) != read_data) {
      handle_error("write");
    }
    if (read_data == -1) {
      handle_error("read");
    ...
    // Increase total msg count per msg
    // Lock total_message_count using mutex
    pthread_mutex_lock(&count_mutex);
    total_message_count++;
    int curr_total_msgs = total_message_count;
    pthread_mutex_unlock(&count_mutex);

    printf("Msg #   %d; Client ID %d: %s\n", curr_total_msgs, client->client_id, buf);
  }
...
int main() {
  ...
  for (;;) {
    ...
    // use mutex to lock client_id_counter
