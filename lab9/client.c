 ...
 * ANS: IP: 127.0.0.1 port #: 8000
 *
 * 2. Is it UDP or TCP? How do you know?
 * ANS: TCP, socket function passes in a sock_stream argument which uses TCP
 *
 * 3. The client is going to send some data to the server. Where does it get this data from? How can
 * you tell in the code?
 * ANS: The while loop which contains a read function taking in input from a keyboard
 *
 * 4. How does the client program end? How can you tell that in the code?
 * ANS: Client program ends by using close() or exit() difference is in that the former ends the
 * network connection while the latter ends the program
 */
#include <arpa/inet.h>
#include <stdio.h>
#include <stdlib.h>
