...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  int found = 0; // Flag to indicate matching value entry
  int curr_index = *result_count;

  // Loop over current output array to see if input value matches any output value
  for (int i = 0; i < *result_count; i++) {
  ...
  // If it doesnt exists add new entry
  if (!found) {
    output[curr_index].doubled_value = input->doubled_value;
    output[curr_index].line_numbers[0] = input->line_number;
    output[curr_index].count = 1;

    // Increment result count to indicate how many unique keys have been found
    (*result_count)++;
  }
}
