 ...
 * ANS: The client sends data to the server using the TCP protocol
 *
 * What data is the client sending to the server?
 * ANS: Char typed messages
 *
 * Understanding the Server:
 * 1. Explain the argument that the 'run_acceptor' thread is passed as an argument.
 * ANS: A multithreaded server is used by the server to handle different client connections from the
 * run_acceptor func
 *
 * 2. How are received messages stored?
 * ANS: Stored as list in structs
 *
 * 3. What does 'main()' do with the received messages?
 * ANS: It collects the receieved messages and stores them in a list
 *
 * 4. How are threads used in this sample code?
 * ANS: Threads are used to handle messages that are sent by each connected client using the
 * run_acceptor function
 *
 * 5a. How are sockets made non-blocking?
 * ANS: From the set_non_blocking function using the fcntl sys-call
 *
 * 5b. What sockets are made non-blocking?
 * ANS: The sfd for ther server listening and the cfd for client connection
 *
 * 5c. Why are sockets made non-blocking? Its purpose?
 * ANS: The sockets are made non-blocking to allow the functions to return regardless of whether its
 * ready or not. For this sample code specifically it is used to terminate threads and allow the
 * loop to run again when no data or connections are found.
 */
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
...
static void *run_acceptor(void *args) {
  ...
  while (aargs->run) {
    if (num_clients < MAX_CLIENTS) {
      ...
      if (cfd == -1) {
        ...
        if (pthread_create(&threads[num_clients], NULL, run_client, &client_args[num_clients]) !=
            0) {
  ...
  for (int i = 0; i < num_clients; i++) {
    ...
    client_args[num_clients].run = false;

    // TODO: Wait for the client thread and close its socket
    if (pthread_join(threads[i], NULL) != 0) {
      perror("Failed to join close client thread!\n");
    }
    if (close(client_args[i].cfd = -1)) {
      perror("Failed to close client thread.\n");
    }
  }
...
int main() {
  ...
  int target_msgs = MAX_CLIENTS * NUM_MSG_PER_CLIENT;

  while (1) {
    pthread_mutex_lock(&list_mutex);
    int curr_count = list_handle.count;
    pthread_mutex_unlock(&list_mutex);

    if (curr_count >= target_msgs) {
      break;
    }
  }
